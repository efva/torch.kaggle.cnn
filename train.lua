...
function Trainer:test(epoch, dataloader)
   ...
   for n, sample in dataloader:run() do
      ...
      local top1, top3 = self:computeScore(output, sample.target, nCrops)
      top1Sum = top1Sum + top1
      top3Sum = top3Sum + top3
      N = N + 1

      print((' | Test: [%d][%d/%d]    Time %.3f  Data %.3f  top1 %7.3f (%7.3f)  top3 %7.3f (%7.3f)'):format(
         epoch, n, size, timer:time().real, dataTime, top1, top1Sum / N, top3, top3Sum / N))
   ...
   print((' * Finished epoch # %d     top1: %7.3f  top3: %7.3f\n'):format(
      epoch, top1Sum / N, top3Sum / N))

   return top1Sum / N, top3Sum / N
end
